...
bool UMyEditorUtilityWidget::Initialize()
{
	...
	{
		DownloadSheetButton->OnClicked.AddDynamic(this,&UMyEditorUtilityWidget::OnDownloadSheetButtonClicked);
	}
	CurrentSheetIndex = 0;
	return true;
}

void UMyEditorUtilityWidget::OnDownloadSheetButtonClicked()
{
...
void UMyEditorUtilityWidget::RequestSheetData(FString SheetId,FString SheetTitle)
{
...
void UMyEditorUtilityWidget::OnRequestComplete(UVaRestRequestJSON* Result)
{
	auto logs = TextLogs->GetText().ToString();
...
TSharedPtr<FJsonObject> UMyEditorUtilityWidget::ParseJsonString(const FString& JsonString)
{
...
void UMyEditorUtilityWidget::AccessJsonData(const TSharedPtr<FJsonObject>& JsonObject)
{
	if (!JsonObject.IsValid())
	{
